...
##df2$ecig_craving<- as.factor(df2$ecig_craving)
...
summary(df2_imputed$R01_AE0103)
summary(df2_imputed$R01_AE0103_imp)
summary(df2$R01_AE0103)
summary(df2_imputed$current_established_ecig_user_imp)
summary(df2_imputed$current_established_ecig_user)
summary(df2$current_everyday_ecig_user)
...
cart_fit2 <- cart_wf2 %>%
  parsnip::fit( data = train2)

y_predict2 <-predict(cart_fit2, new_data = test2) 


rpart.plot(x = cart_fit$fit$fit$fit,fallen.leaves=FALSE, tweak=1.2)
## still the same decision tree 
```
